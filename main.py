...
async def post_trade_on_iqoption(trade: Trade):
    ...
    iq_option = IQOptionAPI("olamidejubril68@gmail.com", "#Obalola21#")
    ...
    if connect_success:
        ...
        raise HTTPException(status_code=400, detail=f"Buying of {trade.actives} failed")
    raise HTTPException(
        status_code=400, detail=f"Connection failed. Reason {connection_failed}"
    )
